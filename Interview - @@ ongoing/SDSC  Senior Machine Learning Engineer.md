...
- CleaningCoagulation（P6）：在局部出血时，引入Bipolar进行[凝血](https://zhida.zhihu.com/search?content_id=188787886&content_type=Article&match_order=1&q=%E5%87%9D%E8%A1%80&zhida_source=entity)和Irrigator进行吸血清理
- GallbladderRetraction（P7）：将打包好的胆囊通过管道拉出人体
- 
![[Pasted image 20250808192253.png]]

手术[流分析](https://zhida.zhihu.com/search?content_id=188787886&content_type=Article&match_order=1&q=%E6%B5%81%E5%88%86%E6%9E%90&zhida_source=entity)属于小众领域，但是，是实现 计算机辅助的介入式系统（Computer-Assisted Interventional System）的重要一环。手术流的应用可以粗略分为四类：

- 手术阶段识别（<mark style="background: #FFF3A3A6;">Surgical Phase Recognition</mark>）：是广泛关注并且取得明显成效的领域，旨在将每一帧图片归类为 **P0-P6** 中的某一个阶段。在有偏差和异常情况下以早期警告的形式向医生提供信息以及提供上下文感知决策。

- 技能评估（<mark style="background: #FFF3A3A6;">Skill Assessment</mark>）：将一整个手术流视频归为 **菜鸟/入门/老手** 等类别，以此来评估整个手术的好坏，从而进行医生培训。

- 手术工具/阶段预测（<mark style="background: #FFF3A3A6;">Surgical Instrument/Phase Anticipation</mark>）：预测下一个会使用的工具以及阶段，以此来进行提前的工具准备，并发症的预防等。

- 剩余时间预测（<mark style="background: #FFF3A3A6;">Remaining Time Prediction</mark>）：预测出整个手术的剩余时间来进行智能手术室管理。

- [场景分割](https://zhida.zhihu.com/search?content_id=188787886&content_type=Article&match_order=1&q=%E5%9C%BA%E6%99%AF%E5%88%86%E5%89%B2&zhida_source=entity)：对视频画面中的物体进行分割，同[计算机视觉](https://zhida.zhihu.com/search?content_id=188787886&content_type=Article&match_order=1&q=%E8%AE%A1%E7%AE%97%E6%9C%BA%E8%A7%86%E8%A7%89&zhida_source=entity)中的分割。


### **第一部分：現有主流方法的回顧**

您提到的方法可以歸納為幾個主流方向，它們之間並非完全獨立，常常會組合使用。

1. **兩階段法：特徵提取器 (CNN/ViT) + 時序模型 (RNN/LSTM)**
    
    - **流程**：這是最經典的方法。
        
        1. **逐幀特徵提取**：將手術影片的每一幀輸入一個強大的圖像特徵提取器，例如在 ImageNet 上預訓練的 ResNet (CNN) 或 Vision Transformer (ViT)。每一幀都被轉換成一個高維特徵向量 (feature vector)。
            
        2. **時序建模**：將這些幀特徵向量組成的序列，輸入到一個時序模型中，如 LSTM、GRU 或 TCN (Temporal Convolutional Network)。時序模型負責學習不同幀之間的依賴關係和時間模式。
            
        3. **分類**：時序模型的輸出再經過一個分類層，來判斷每一幀或每一段時間屬於哪個手術階段 (phase)。
            
    - **本質**：將空間特徵和時間特徵的學習**解耦**，分開處理。
        
2. **端到端法：3D-CNN 或 Video Vision Transformer (ViViT)**
    
    - **流程**：這類模型可以直接處理影片片段 (video clips)，同時學習時空特徵，無需分成兩步。
        
        - **3D-CNN** (如 I3D, C3D)：使用 3D 卷積核，同時在空間（高、寬）和時間維度上進行卷積，直接捕捉動態模式。
            
        - **ViViT** (如前述)：將影片切成 3D 的 "Tubelets"，利用 Transformer 的自註意力機制同時捕捉空間和時間上的長距離依賴關係。
            
    - **本質**：**端到端 (End-to-End)** 的時空特徵學習。
        
3. **基於感知的輔助方法：物件偵測或分割 (Object Detection/Segmentation)**
    
    - **流程**：這種方法認為，手術階段與**手術器械 (instrument) 的使用**和**解剖結構 (anatomy) 的狀態**高度相關。
        
        1. **偵測/分割**：先用專門的模型對每一幀進行物件偵測（框出所有手術器械）或語義分割（像素級標示出不同器械和器官）。
            
        2. **提取依據**：將偵測/分割的結果作為判斷依據。例如，可以將「偵測到的器械列表」或「器械與組織的接觸關係」作為額外特徵，與影像特徵一同輸入到時序模型中。
            
    - **本質**：引入更細粒度 (fine-grained) 的語義資訊，讓模型不僅「看見」像素，還能「理解」畫面中有什麼、在做什麼。
        

---

### **第二部分：AI 在手術工作流分析的新進展與潛力方法**

除了上述方法，近年來學術界和工業界正在探索以下幾個更前沿的方向：

#### **1. 圖神經網絡 (Graph Neural Networks, GNNs)**

- **核心思想**：將手術場景建模為一個動態的「圖」(Graph)。圖中的**節點 (Nodes)** 可以是手術器械、解剖結構、甚至抽象的動作；**邊 (Edges)** 則代表它們之間的交互關係（如「鉗子夾住紗布」、「手術刀正在切割組織」）。手術工作流就變成了這個交互關係圖隨時間演變的過程。
    
- **為什麼有潛力？**：
    
    - **更豐富的關係建模**：相比於簡單的特徵序列，圖結構能更明確、更靈活地表達器械之間、器械與組織之間的複雜交互關係。
        
    - **更強的解釋性**：可以追蹤圖中特定節點和邊的變化，來解釋為什麼模型會判斷為某個階段。例如，當「持針器」和「縫合線」節點被激活，並且它們之間產生「交互」的邊時，模型會傾向於輸出「縫合」階段。
        
- **實現方式**：先用物件偵測模型找出畫面中的器械和器官作為節點，然後用 GNN 來學習和推理這個圖的演變，最終進行階段分類。
    

#### **2. 弱監督與自監督學習 (Weakly- and Self-Supervised Learning)**

- **核心挑戰**：要給手術影片做逐幀的階段標註，成本極高，需要外科專家花費大量時間。這是推動研究發展的最大瓶頸。
    
- **弱監督學習 (Weakly Supervised Learning)**：
    
    - **思想**：使用「比較弱」的標籤來訓練模型。例如，我們不標註每一幀，只提供一個影片的**階段順序列表**（如 `['切口', '分離', '縫合']`）。
        
    - **方法**：利用類似於語音辨識中的 CTC Loss (Connectionist Temporal Classification) 等技術，讓模型自己去學習將這個順序列表與影片的時間軸對齊。
        
- **自監督學習 (Self-Supervised Learning)**：
    
    - **思想**：完全不使用人工標籤，讓模型從海量的未標註影片中自己學習通用的時空特徵。
        
    - **方法**：設計一些「藉口任務」(Pretext Task)，例如：
        
        - **時序預測**：給模型影片的前半段，讓它預測後半段的樣子。
            
        - **時序排序**：將影片片段打亂，讓模型學習恢復它們的正確順序。
            
        - **對比學習 (Contrastive Learning)**：讓模型學習區分「同一手術中的不同片段」和「不同手術的片段」。
            
    - **應用**：通過自監督學習預訓練後得到的模型，再用少量有標籤的數據進行微調 (fine-tune)，就能在階段辨識任務上達到很好的效果，大大降低對標籤數據的依賴。
        

#### **3. 多模態融合 (Multimodal Fusion)**

- **核心思想**：手術室中不僅有影像，還有來自各種設備的數據流。將這些多源數據融合起來，可以提供比單純影像更可靠的判斷依據。
    
- **可融合的數據模態**：
    
    - **機器人運動學數據 (Kinematic Data)**：對於達文西等手術機器人，可以獲取機械臂的位置、姿態、速度、夾持力等高頻精確數據。器械的運動模式是區分階段的極強信號。
        
    - **系統事件日誌 (System Event Logs)**：例如電刀、超聲刀等能量設備的啟用/關閉記錄。
        
    - **內窺鏡影像數據 (Video Data)**：即傳統的影像流。
        
    - **術前醫學影像 (Pre-operative Data)**：如病患的 CT、MRI，可以提供解剖結構的先驗知識。
        
- **為什麼有潛力？**：不同模態的數據可以互為補充。例如，當影像中因為煙霧看不清時，機器人的運動數據和能量設備日誌仍然可以準確地指示當前正在進行電灼止血操作。
    

#### **4. 可解釋性AI (Explainable AI, XAI) 與不確定性量化 (Uncertainty Quantification)**

- **核心思想**：在醫療這種高風險領域，AI模型不僅要「做得對」，還必須「讓人信服」。黑盒子模型很難被醫生接受。
    
- **新進展**：
    
    - **可解釋性**：利用注意力圖 (Attention Maps) 等技術，視覺化模型在做決策時，究竟是關注了影像中的哪個區域或哪些器械。例如，在判斷為「縫合」時，模型應該高度關注持針器和縫合線。
        
    - **不確定性量化**：模型在輸出預測結果的同時，也給出一個「信賴度」分數。當模型對自己的判斷不確定時（例如，遇到罕見情況或畫面模糊），它可以主動發出警報，提示需要人工介入。這對於臨床安全至關重要。
...
| 方法 (Method)                                    | 核心思想 (Core Idea)               | 優點 (Advantages)                                      | 缺點 (Disadvantages)                                        |
| ---------------------------------------------- | ------------------------------ | ---------------------------------------------------- | --------------------------------------------------------- |
| **CNN/ViT + RNN/LSTM**<br>(兩階段法)               | 空間與時間特徵解耦學習。先提取幀特徵，再用時序模型串聯。   | - 概念清晰，實現相對簡單。<br>- 可以利用強大的預訓練圖像模型 (ResNet/ViT)。     | - 兩階段流程可能不是最優的。<br>- RNN/LSTM 對超長序列的建模能力有限。               |
| **End-to-End Video Models**<br>(3D-CNN, ViViT) | 直接在時空維度上進行端到端學習，同時捕捉空間和時間特徵。   | - 理論上能學到更優的時空聯合特徵。<br>- 模型結構更統一。                     | - 計算量巨大，對硬體要求高。<br>- 需要非常大的影片數據集來訓練。                      |
| **基於物件/工具的方法**                                 | 引入手術器械、解剖結構等細粒度語義資訊作為判斷依據。     | - 提供了更豐富的資訊，通常能顯著提升準確率。<br>- 模型決策更符合人類醫生的邏輯。         | - 依賴於一個額外且準確的物件偵測/分割模型。<br>- 增加了系統的複雜性。                   |
| **圖神經網絡 (GNNs)**                               | 將手術場景建模為動態的「交互關係圖」，學習圖結構的演變。   | - 能明確地建模複雜的物體間交互關係。<br>- 模型具有更好的結構性和可解釋性。            | - 圖的構建（如何定義節點和邊）是關鍵且具挑戰性。<br>- 相關研究尚處於早期階段。               |
| **弱監督/自監督學習**                                  | 使用弱標籤或無標籤數據進行訓練，以降低對昂貴精細標註的依賴。 | - **極大地降低數據標註成本**，解決了最大瓶頸。<br>- 能利用海量未標註數據，學習更泛化的特徵。 | - 弱監督方法的精度可能略低於全監督。<br>- 自監督的藉口任務設計需要技巧和專業知識。             |
| **多模態融合**                                      | 結合影像、機器人運動學、系統日誌等多源數據進行綜合判斷。   | - 系統魯棒性極強，不易被單一數據源的噪聲干擾。<br>- 能提供最全面的資訊，預測精度潛力最高。    | - 數據採集和同步對齊非常複雜。<br>- 僅適用於配備了相應設備的手術室（如手術機器人）。            |
| **可解釋性AI (XAI)**                               | 讓模型的決策過程透明化，並量化其預測的不確定性。       | - 增強臨床醫生對AI的信任度，是推向臨床應用的關鍵。<br>- 提升系統的安全性。           | - 這是一個研究方向，而非獨立的分類方法，需要與上述方法結合。<br>- 實現好的解釋性和不確定性量化本身有難度。 |
...
弱監督與自監督學習是解決手術影片分析中「數據標註瓶頸」的關鍵技術。以下我將為您詳細解釋這兩種方法，包含數據集、模型、訓練、推論以及優缺點的完整說明。

---

### **前言：為何需要弱監督與自監督學習？**

傳統的全監督學習 (Fully-Supervised Learning) 需要對手術影片的**每一幀**都標註其對應的手術階段。一場數小時的手術包含數十萬幀畫面，標註工作耗時、昂貴，且需要頂尖的外科專家才能完成，這極大地限制了數據集的規模和模型的發展。弱監督和自監督學習正是為了解決這個根本問題而生。

---

### **一、弱監督學習 (Weakly-Supervised Learning, WSL)**

#### **核心思想**

弱監督學習的核心是**使用「不精確」但「成本低廉」的標籤來進行訓練**。我們不再追求逐幀的完美標註，而是利用一些概括性的、粗糙的標籤，讓模型自己去學習將這些弱標籤與影片的具體內容對應起來。

#### **數據集 (Datasets)**

- **大小 (Size)**：通常包含數百到上千場手術。由於標註成本降低，數據集規模可以比全監督大得多。例如，一個研究可能使用 1000 支腹腔鏡膽囊切除術的影片。
    
- **格式 (Format)**：
    
    1. **影片文件**：標準的 `.mp4` 或其他影片格式。
        
    2. **標籤文件**：一個簡單的文本文件或 JSON 文件。
        
- **標籤 (Label)**：最常見的弱標籤是 **「手術階段文字稿」(Phase Transcripts)**。
    
    - **格式**：對於每一支影片，標籤不是 `(影格編號, 階段)` 的對應表，而只是一個**按時間順序排列的階段列表**。
        
    - **範例**：對於影片 `surgery_001.mp4`，其標籤可能就是 `["Preparation", "Incision", "Dissection", "Suturing", "Conclusion"]`。它只告訴我們這場手術經歷了這五個階段，以及它們的先後順序，但**沒有提供每個階段具體的開始和結束時間點**。
        
- **製作方法 (How to Create)**：
    
    - 標註者（可以是經驗較淺的醫學生或助理）只需觀看一遍手術影片。
        
    - 他們不需要暫停影片去精確定位時間點，只需在觀察到階段轉換時，記錄下該階段的名稱。
        
    - 整個過程可能只需要 5-10 分鐘，而全監督標註則需要數小時。
        

#### **具體實現範例**

**目標**：訓練一個模型，用上述的「階段文字稿」標籤，來實現逐幀的手術階段辨識。

- **模型建議 (Model Suggestion)**：
    
    1. **影像特徵提取器 (Feature Extractor)**：一個在大型圖像數據集（如 ImageNet）上預訓練過的 CNN 模型（如 `ResNet-50`）或 ViT 模型。它負責將影片的每一幀轉換成一個高維特徵向量。
        
    2. **時序模型 (Temporal Model)**：一個雙向 LSTM (Bi-LSTM) 或 Transformer Encoder。它接收特徵向量序列，並學習它們之間的時間關係。
        
    3. **分類層 (Classification Layer)**：一個線性層，將時序模型的每個時間步的輸出轉換為在所有可能階段（包含一個特殊的 `blank` 標記）上的機率分佈。
        
- **訓練過程 (Training Process)**：
    
    - **關鍵技術：CTC Loss (Connectionist Temporal Classification Loss)**。CTC 源於語音辨識，非常適合處理「輸入序列長度遠大於輸出標籤長度」的場景。
        
    
    1. **前向傳播**：將一支影片的所有幀通過模型，得到一個幀級的機率序列 `P`（例如，長度為 100,000 幀）。
        
    2. **對齊**：CTC Loss 會聰明地計算出所有可能的、能將這個長度為 100,000 的機率序列 `P` 對應到短的目標標籤 `["Incision", "Dissection", ...]` 的路徑。例如，它會考慮「前 15,000 幀是 Incision，後 20,000 幀是 Dissection...」這種對齊方式的總機率。
        
    3. **反向傳播**：模型會最大化所有正確對齊路徑的總機率，從而反向傳播梯度來更新模型權重。
        
    
    - **效果**：通過這種方式，模型被迫自己學習**在哪個時間段內，「Incision」的機率應該最高**，以及**階段轉換的邊界大概在哪裡**，儘管我們從未明確告訴它這些資訊。
        
- **推論結果 (Inference Output)**：
    
    - 訓練完成後，輸入一支新的影片。
        
    - 模型會輸出一個逐幀的機率分佈。
        
    - 使用**貪婪解碼 (Greedy Decoding)**（即在每一幀選擇機率最高的階段）或更複雜的**束搜索 (Beam Search)**，就可以得到**逐幀的手術階段預測結果**。其格式與全監督模型完全一樣，例如 `frame_001: Incision, frame_002: Incision, ..., frame_15000: Dissection`。
...
自監督學習比弱監督更進一步，它在**預訓練階段完全不使用任何人工標籤**。它通過設計一些「藉口任務」(Pretext Tasks)，讓模型從數據本身的結構中學習通用的、有意義的視覺特徵。學到的好特徵可以極大提升後續在少量標籤數據上的學習效果。

#### **數據集 (Datasets)**

- **預訓練數據集 (Pre-training Dataset)**：
    
    - **大小**：**極大規模**，可以包含數萬小時的影片，因為它不需要任何標籤。可以輕易地從醫院的影片存檔中獲取。
        
    - **格式**：只有影片文件。
        
    - **標籤**：**無 (None)**。
        
    - **製作方法**：只需收集和整理影片，無需任何標註工作。
        
- **微調數據集 (Fine-tuning Dataset)**：
    
    - **大小**：**非常小**，可能只有幾十場手術的精細標註數據（即全監督標籤）。
        
    - **格式與標籤**：與傳統全監督學習的數據集完全一樣。
        

#### **具體實現範例**

**目標**：利用海量無標籤影片預訓練一個強大的特徵提取器，然後在少量有標籤的數據上進行微調，以實現高精度的階段辨識。

- **模型建議 (Model Suggestion)**：
    
    - 一個強大的影片模型骨幹，如 **3D-ResNet** 或 **Video Vision Transformer (ViViT)**。
        
- **訓練過程 (Training Process)**：這是一個兩階段過程。
    
    **階段一：自監督預訓練 (Pre-training on unlabeled data)**
    
    - **關鍵技術：影片對比學習 (Video Contrastive Learning)**，例如 `SimCLR` 或 `MoCo` 的影片版本。
        
    
    1. **數據準備**：從一支無標籤的手術影片中，隨機採樣一個影片片段，稱之為**錨點 (Anchor)**。
        
    2. **正樣本 (Positive Sample)**：從**同一支影片**中，採樣另一個時間上鄰近的片段，並對其進行數據增強（如隨機裁剪、顏色抖動）。這個片段與錨點在語義上是相似的（可能都屬於同一手術階段）。
        
    3. **負樣本 (Negative Samples)**：從**許多其他不同的手術影片**中隨機採樣多個片段。這些片段與錨點在語義上是無關的。
        
    4. **學習目標**：將所有樣本輸入模型，得到它們的特徵向量。模型的目標是，在特徵空間中，**拉近**錨點和正樣本的距離，同時**推遠**錨點和所有負樣本的距離。
        
    5. **效果**：通過在數百萬個這樣的正負樣本對上進行訓練，模型被迫學習到什麼樣的視覺模式（如器械的運動、組織的形變）是區分不同手術場景的關鍵。它學到的是一種通用的、高質量的手術影片特徵表示。
        
    
    **階段二：監督微調 (Fine-tuning on labeled data)**
    
    6. **加載模型**：取在第一階段預訓練好的模型骨幹。
        
    7. **添加分類頭**：在模型骨幹的頂部添加一個簡單的線性分類層。
        
    8. **訓練**：使用少量精細標註的數據來訓練這個分類頭（以及選擇性地微調骨幹網絡的最後幾層）。由於模型已經具備了強大的特徵提取能力，它能很快地在小數據集上學會階段分類任務，且效果遠超從零開始訓練。
        
- **推論結果 (Inference Output)**：
    
    - 微調完成後，模型的推論過程與標準的全監督模型完全相同。
...
|特性|**弱監督學習 (Weakly-Supervised Learning)**|**自監督學習 (Self-Supervised Learning)**|
|---|---|---|
|**核心目標**|使用低成本、不精確的標籤直接完成下游任務（如階段辨識）。|利用無標籤數據學習一個通用的、強大的特徵表示，以輔助下游任務。|
|**數據標註成本**|**低**。只需提供階段順序等弱標籤，比全監督便宜得多。|**預訓練階段為零**。微調階段需要少量昂貴的全監督標籤。|
|**所需數據量**|需要**大量**的弱標籤數據來驅動學習（如數百上千場手術）。|**預訓練數據多多益善**（可達數萬場），微調數據量可以**非常小**。|
|**模型與訓練複雜度**|**中等**。需要實現像 CTC Loss 這樣的特定算法，但流程是端到端的。|**高**。訓練分為預訓練和微調兩個獨立階段，對比學習的實現較為複雜。|
|**潛在性能**|性能通常**略低於**全監督方法，但遠高於從零訓練，且性價比高。|有潛力**接近甚至超越**全監督方法，特別是當無標籤數據極其龐大時。|
|**優點**|- 直接解決最終任務，流程相對直接。<br>- 標註成本和性能之間的平衡點較好。<br>- 適用於已有大量弱標籤數據的場景。|- **最大化地利用了無標籤數據**，徹底打破數據瓶頸。<br>- 學到的特徵更泛化、更魯棒，可遷移到多個下游任務。<br>- 在標籤數據極度稀缺時，優勢巨大。|
|**缺點**|- 仍然需要一定的人工標註。<br>- 性能上限受限於弱標籤的資訊量。<br>- 對於非常複雜的工作流，僅靠順序標籤可能不足。|- 訓練流程更複雜，分為兩個階段。<br>- 需要巨大的計算資源進行預訓練。<br>- 藉口任務的設計對最終性能影響很大。|
|**適用場景**|當你有能力獲取大量（數百上千）手術的粗略標註（如階段順序），但無法進行精細標註時。|當你擁有海量（成千上萬）未標註的手術影片，但只有極少量（幾十個）的精細標註數據時。|

#### CTC Loss (Connectionist Temporal Classification Loss)

带你看懂CTC算法 - 佛系小僧的文章 - 知乎
https://zhuanlan.zhihu.com/p/161186907

CTC loss 笔记+源码分享 - 寒夏凉秋的文章 - 知乎
...
在手術工作流分析中使用圖神經網絡 (Graph Neural Networks, GNNs) 的方法。這是一種非常前沿且富有潛力的技術，它將分析的視角從「像素流」轉變為「物體交互關係」。

---

### **核心思想：從像素到關係**

傳統方法（如 CNN+LSTM 或 ViViT）將影片視為一個個像素網格的時間序列。而 GNN 方法的核心思想是，**手術場景的本質不是像素，而是器械、器官等實體之間的交互關係**。一個手術階段（Phase）的定義，往往取決於「什麼器械」正在對「哪個器官」執行「何種操作」。

GNN 正是為處理這種「關係」數據而生的。它將手術場景建模為一個**動態的圖 (Dynamic Graph)**，其中：

- **節點 (Nodes)**：代表場景中的實體，如手術剪、持針器、膽囊、肝臟等。
    
- **邊 (Edges)**：代表實體之間的關係，如「器械A在器械B附近」（空間關係）或「器械A正在接觸組織C」（交互關係）。
    

整個手術工作流就變成了這個「物體交互圖」隨時間演變的序列。GNN 的任務就是學習這個圖序列的模式，從而判斷當前處於哪個手術階段。

---

### **一、數據集 (Datasets)**

GNN 方法對數據集的標註要求是**所有方法中最高、最昂貴的**，因為它需要結構化的語義資訊來構建圖。

- **大小 (Size)**：通常也是數百場手術的規模。代表性的公開數據集是 **CholecT50**，它包含了 50 場腹腔鏡膽囊切除術的影片，並提供了 GNN研究所需的豐富標註。
    
- **格式 (Format)**：數據集通常包含：
    
    1. **影片文件**：標準的 `.mp4` 影片。
        
    2. **標註文件**：通常是 `.json` 或 `.xml` 格式，每一幀都包含以下資訊：
        
        - **物件標註 (Object Annotations)**：所有可見的**手術器械 (Instrument)** 和 **解剖結構 (Anatomy)** 的標註。這通常是**邊界框 (Bounding Box)** 或更精確的**分割掩碼 (Segmentation Mask)**。
            
        - **物件類別 (Object Class)**：每個邊界框或掩碼對應的類別，如 `Grasper` (抓鉗), `Scissors` (剪刀), `Gallbladder` (膽囊) 等。
            
        - **階段標籤 (Phase Label)**：該幀對應的全局手術階段標籤，如 `Dissection` (分離)。
            
- **標籤 (Label)**：GNN 方法需要**兩個層級的標籤**：
    
    1. **用於構建圖的標籤**：每一幀中所有器械和器官的**位置和類別**。
        
    2. **用於最終任務的標籤**：每一幀的**手術階段**。
        
- **製作方法 (How to Create)**：
    
    1. 標註專家需要逐幀觀看影片。
        
    2. 在每一幀中，用繪圖工具為畫面裡**每一個**可見的器械和關鍵器官繪製精確的邊界框或分割掩碼。
    ...
    - **這是一個極其耗時且昂貴的過程**，其成本遠高於僅標註手術階段的全監督方法。
...
GNN 方法通常是一個多階段的流水線 (Pipeline)。

**目標**：訓練一個基於 GNN 的模型，輸入手術影片，輸出逐幀的階段預測。

#### **模型與訓練流程**

**第 1 步：場景感知 - 物件偵測/分割**

- **任務**：在每一幀畫面中，準確地定位所有的手術器械和解剖結構。
    
- **模型建議**：
    
    - **物件偵測**：使用 `YOLOv8` 或 `Faster R-CNN` 等模型，它們可以在畫面中為每個物件生成邊界框和類別。
        
    - **實例分割**：使用 `Mask R-CNN`，可以生成更精確的像素級掩碼，有助於判斷物體間的接觸關係。
        
- **訓練**：這個偵測/分割模型需要先在帶有物件標註的數據集上進行**預訓練**。
    

**第 2 步：動態圖構建 (Dynamic Graph Construction)**

- **任務**：對於每一幀，利用上一步的偵測結果來構建一個圖 `G_t`。
    
- **如何構建**：
    
    1. **定義節點 (Nodes)**：將第 1 步中偵測到的每一個物件（如抓鉗、剪刀、膽囊）作為圖的一個節點。
        
        - **節點特徵 (Node Features)**：每個節點的初始特徵可以來自偵測模型對應區域的視覺特徵（RoI features），或者是一個代表其類別的獨熱編碼 (one-hot vector)。
            
    2. **定義邊 (Edges)**：根據預設規則連接節點。規則可以有很多種，例如：
        
        - **空間鄰近邊**：如果兩個物件的邊界框中心距離小於某個閾值，或它們的 IoU (交併比) 大於某個閾值，則在這兩個節點間添加一條邊。
            
        - **交互關係邊**：如果一個「器械」節點的掩碼與一個「組織」節點的掩碼有重疊，則添加一條代表「交互」的邊。
            
        - **全連接邊**：最簡單的方式，將所有節點兩兩相連，讓模型自己去學習邊的重要性。
            
    
    - 這個過程對影片的每一幀都重複執行，最終得到一個**圖序列 (a sequence of graphs)** `{G_1, G_2, ..., G_T}`。
        

**第 3 步：時空圖學習 (Spatio-Temporal Graph Learning)**

- **任務**：學習這個圖序列的演變模式。
    
- **模型建議**：一個能夠處理圖序列的模型，常見組合是 **GNN + RNN**。
    
    - **空間聚合 (Spatial Aggregation)**：對於序列中的每一個圖 `G_t`，使用一個標準的 GNN 模型（如 `GCN`, `GAT` 或 `GraphSAGE`）來更新每個節點的表示。GNN 會聚合每個節點鄰居的資訊，從而讓節點的特徵不僅包含自身資訊，還包含了它與周圍物件的**關係資訊**。
        
    - **時間聚合 (Temporal Aggregation)**：將每一時刻 `t` 經過 GNN 更新後的節點特徵進行池化 (Pooling)，得到一個代表該時刻全圖狀態的向量 `g_t`。然後，將這個圖級特徵序列 `{g_1, g_2, ..., g_T}` 輸入到一個時序模型中（如 `LSTM` 或 `GRU`）。
        
    - **模型架構**：`GCN-LSTM` 或 `GAT-LSTM` 是常見的組合。
        

**第 4 步：階段分類 (Phase Classification)**

- **任務**：根據學習到的時空圖特徵進行最終的分類。
    
- **實現**：將 LSTM 在每個時間步的隱狀態 (hidden state) 輸入到一個線性分類層（帶有 Softmax 激活函數），以預測該時刻對應的手術階段。
    
- **損失函數**：使用標準的交叉熵損失 (Cross-Entropy Loss) 來比較預測結果和真實的階段標籤，並進行反向傳播更新整個模型（從 GNN 到 LSTM 再到分類層）的權重。
...
- 對每一幀，依次執行**物件偵測 -> 圖構建 -> GNN空間聚合 -> LSTM時間更新 -> 分類**。
    
- 最終的輸出是一個**逐幀的手術階段預測序列**，例如 `frame_001: Preparation, frame_002: Preparation, ..., frame_30000: Dissection`。
...
將 GNN 方法與傳統的影像序列方法（如 CNN+LSTM 或 ViViT）進行比較：

|特性|**GNN 方法 (GNN Approach)**|**傳統影像序列方法 (e.g., CNN+LSTM, ViViT)**|
|---|---|---|
|**核心思想**|將場景建模為**物體間的交互關係圖**，學習圖結構的演變。|將影片建模為**像素網格的時間序列**，學習時空特徵。|
|**數據依賴**|**極高**。需要逐幀的物件級標註（邊界框/掩碼）和階段標籤。|**高**。需要逐幀的階段標籤，但不需要物件級標註。|
|**模型可解釋性**|**強 (Strong)**。可以通過分析圖的邊和節點注意力來解釋決策。例如，可以視覺化模型在判斷為「剪切」時，是否關注了「剪刀」節點與「組織」節點之間的交互。|**弱 (Weak)**。通常依賴於Saliency Map或注意力圖來顯示像素級的重要性，解釋較為抽象，不直接關聯到具體物體。|
|**計算複雜度**|**高**。是一個多階段流水線，包含了物件偵測、圖構建和圖神經網絡計算。|**中到高**。端到端模型（如 ViViT）雖然結構統一，但本身計算量也很大。|
|**性能瓶頸**|**受限於上游的物件偵測器**。如果器械或器官沒有被準確檢測出來，圖本身就是錯的，後續的 GNN 也無能為力。|性能直接與模型自身的時空特徵提取能力相關。|
|**優點**|- **更符合人類認知**，顯式地對物體交互進行建模。<br>- **可解釋性強**，有助於建立臨床信任。<br>- 對於由特定工具交互定義的階段，可能更魯棒。|- **端到端的方法更簡潔**，避免了複雜的流水線。<br>- **數據標註成本相對較低**。<br>- 技術更成熟，有大量預訓練模型可用。|
|**缺點**|- **數據標註成本極其昂貴**，是目前最大的落地障礙。<br>- **多階段流水線容易產生誤差累積**。<br>- 實現和調優更複雜。|- **可解釋性較差**，像一個「黑盒子」。<br>- 可能會學習到一些偽相關性（spurious correlations），而不是真正的因果關係。|
