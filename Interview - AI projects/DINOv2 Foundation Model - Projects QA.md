...
- **分解矩陣更新**
    - 假設原模型的權重矩陣為 WWW，LoRA 通過添加一個低秩矩陣 ΔW\Delta WΔW 來對權重進行微調： W′=W+ΔW=W+A×BW' = W + \Delta W = W + A \times BW′=W+ΔW=W+A×B 其中：
        - AAA 是低秩矩陣（形狀為 d×rd \times rd×r），rrr 遠小於 ddd。
        - BBB 是另一個低秩矩陣（形狀為 r×kr \times kr×k）。
- **優勢：**
    - 原始權重 WWW 保持不變，僅更新小規模的增量參數 AAA 和 BBB。
...
假設一個 Transformer 層的線性層權重為 W∈R1024×1024W \in \mathbb{R}^{1024 \times 1024}W∈R1024×1024，應用 LoRA：

- 原始參數量為 10242=1,048,5761024^2 = 1,048,57610242=1,048,576。
- 如果選擇 r=16r = 16r=16，則新增參數量為： 1024×16+16×1024=32,7681024 \times 16 + 16 \times 1024 = 32,7681024×16+16×1024=32,768 僅占原參數量的 3%。
