...
def min_sliding_window(nums, k):
    ...
    for i in range(len(nums)):
        if dq and dq[0] <= i - k:
            dq.popleft()
        while dq and nums[dq[-1]] >= nums[i]: # 和對尾dq[-1]相比更小,可大可小
            dq.pop()
        dq.append(i)
...
def min_sliding_window(nums, k):
    ...
    results = []
    # 單調佇列，儲存的是元素的索引。
    # 佇列中的索引對應的元素值從隊頭到隊尾是遞增的。
    dq = deque()

    # 遍歷數組
    for i in range(len(nums)):
        # 1. 移除佇列頭部「過期」的索引
        # 如果隊頭的索引 (dq[0]) 已經超出當前窗口的左邊界 (i - k)，則將其移除。
        if dq and dq[0] <= i - k: # 注意：i - k 是窗口最左邊元素的「前一個」索引
            dq.popleft()

        # 2. 維護單調性：移除隊尾所有比當前元素大的索引
        # 如果隊尾的索引對應的值比當前元素 nums[i] 大，
        # 那麼這個隊尾元素不可能成為後續窗口的最小值，因為 nums[i] 更小且更靠近右邊。
        ...
        # 4. 當窗口完全形成後（即遍歷到第 k-1 個元素之後），將當前窗口的最小值加入結果
        # 佇列的隊頭元素始終是當前窗口中的最小值（因為佇列是單調遞增的）。
        if i >= k - 1:
            results.append(nums[dq[0]])
...
k = 3
print(f"原始陣列: {nums}, 視窗大小: {k}")
print(f"滑動視窗最小值: {min_sliding_window(nums, k)}")
# 預期輸出: [ -1, -3, -3, -3, 3, 3 ]
