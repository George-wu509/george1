...
|                                  |                                              |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| -------------------------------- | -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Step1<br>Object detection        | Multi-Object <br>detection<br>(Every Camera) | YOLO                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Step2<br>Object tracking<br>     | Multi-Object <br>Tracking<br>(Every Camera)  | SORT<br>DeepSORT<br>ByteTrack                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Step3<br>Object tracking<br><br> | Multi-Camera<br>Multi-Object <br>Tracking    | 1. 攝影機校正（Camera Calibration） <br>   時間同步（Time Synchronization）<br><br>2. 局部軌跡片段生成 <br>   (Tracklet Generation)<br><br>3. 跨攝影機軌跡片段關聯 <br>   (Inter-Camera Tracklet Association)<br>   -> 外觀相似度 (Appearance Similarity)<br>      用匈牙利演算法 (Hungarian Algorithm) <br><br>4. 全域ID分配與軌跡縫合 <br>   (Global ID Assignment and Trajectory Stitching)<br>   ->  一旦 tracklets 被成功關聯起來，<br>       就給它們分配一個全域ID<br><br>5. 後處理與優化 <br>   (Post-processing and Refinement)<br>   -> 處理衝突,填補間隙, 平滑化<br> |
|                                  |                                              |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
|           | Tracking algorithm                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SORT      | 先利用object detection model譬如YOLOv8 將video每一幀object detection的bounding box輸入model, 並給予一個id. 在下一幀(t+1)利用<mark style="background: #FFF3A3A6;">kalman filter</mark>預測在下一幀(t+1)的bounding box位置, 並與這一幀(t)的現有bounding box比對. 用<mark style="background: #FFB86CA6;">hungarian algorithm</mark>建立成本矩陣考慮運動資訊, 將下一幀(t+1)的bounding box給予適當的id. , 並用ReID管理更新id                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| DeepSORT  | 先利用object detection model譬如YOLOv8 將video每一幀object detection的bounding box輸入model, 並給予一個id. 而對每個boundary box用Resnet model提取**feature map**. 在下一幀(t+1)利用kalman filter預測在下一幀(t+1)的bounding box位置 也計算<mark style="background: #BBFABBA6;">feature map</mark>, 並與這一幀(t)的現有bounding box feature map比對. 用<mark style="background: #FFB86CA6;">hungarian algorithm</mark>建立成本矩陣考慮運動資訊和**外觀資訊**, 將下一幀(t+1)的bounding box給予適當的id, 並用ReID管理更新id                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| 比較        | **SORT (Simple Online and Realtime Tracking):**<br>    - 它以簡單、高效的著稱，主要使用<mark style="background: #FFF3A3A6;">卡爾曼濾波器（Kalman Filter）</mark>來預測物體在下一幀的位置，並利用匈牙利算法（Hungarian algorithm）將預測結果與檢測結果進行關聯。<br>    - SORT 的優點是速度快，但由於僅基於物體的位置資訊，因此在物體發生遮擋或運動不規則時，追蹤效果可能會下降。<br><br>**DeepSORT (Deep Simple Online and Realtime Tracking):**<br>    - DeepSORT 是在 SORT 的基礎上進行改進，它引入了<mark style="background: #FFB86CA6;">深度學習技術，利用物體的外觀特徵（Appearance Features）來輔助追蹤</mark>。<br>    - 通過使用深度關聯度量（Deep Association Metric），DeepSORT 能夠更準確地處理物體遮擋和 ID 切換（ID Switch）等問題，提高追蹤的魯棒性。<br><br>**ByteTrack (Multi-Object Tracking by Associating Every Detection Box):**<br>    - ByteTrack 的創新之處在於它充分利用了所有檢測框，而不僅僅是高置信度的檢測結果。<br>    - 它通過<mark style="background: #BBFABBA6;">將低置信度的檢測框與高置信度的追蹤軌跡進行關聯</mark>，有效地減少了物體被遮擋或運動模糊時的 ID 切換問題。<br>    - ByteTrack 著重處理被遮蔽物體的追蹤，大大增加了整體追蹤的準確度。 |
|           |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
- **視角差異：**
    ...
    - -> 攝影機校正（Camera Calibration）, 物件軌跡融合（Trajectory Fusion）
- **重疊區域：**
    - 不同攝影機的視野可能存在重疊區域，需要處理物件在這些區域的轉換和關聯。
    - -> 物件軌跡融合（Trajectory Fusion）, 處理重疊區域
- **時間同步：**
...
2. **物件軌跡融合（Trajectory Fusion）：**
    ...
    - 詳細:
     **基於位置的關聯：**
    - 卡爾曼濾波器（Kalman Filter）：用於預測和平滑物件在全局座標系中的運動軌跡。
    - 匈牙利演算法（Hungarian Algorithm）：用於在不同攝影機的軌跡之間建立最佳的關聯。
    - 利用幾何關係：如果攝影機校正結果足夠精確，可以使用多視角之間的幾何關係來限制物體的空間位置，進而提高軌跡的準確度。
	- **基於外觀的關聯：**
    - 物件重識別（ReID）：使用深度學習模型提取物件的外觀特徵，並在不同攝影機的視角下進行匹配。
    - 使用深度特徵進行軌跡匹配，透過計算軌跡之間特徵向量的相似度來找出相同物體的軌跡。
	- **多感測器融合：**
    - 在一些情況下，MCMOT系統可能會結合其他感測器的數據（例如雷達、激光雷達）。使用多感測器融合技術，例如貝葉斯濾波器（Bayesian Filters），將來自不同感測器的資訊組合起來。
    
3. **時間同步（Time Synchronization）：**
    - 確保所有攝影機的影片時間戳記對齊，或者使用時間同步算法來校正時間差異。
    - 硬體同步：使用同步觸發器或網路時間協議（NTP）等硬體設備，確保所有攝影機以相同的時間間隔擷取影像。
...
4. **物件重識別（ReID）：**
    ...
    - 可以使用進階的 ReID 技術，例如：
     ...
     詳細: 
	- **深度學習特徵提取：**
    - 使用 CNN 模型（例如 ResNet、EfficientNet）提取物件的外觀特徵。
    - 訓練 ReID 模型，使其能夠產生視角不變的特徵表示。
	- **度量學習（Metric Learning）：**
    - 使用三元組損失（Triplet Loss）或對比損失（Contrastive Loss）等度量學習技術，訓練 ReID 模型以區分不同的物件。
	- **跨攝影機 ReID：**
    - 開發專門用於跨攝影機物件匹配的 ReID 模型，這些模型能夠處理視角變化和光照差異。
...
带你入门多目标跟踪（三）匈牙利算法&KM算法 - ZihaoZhao的文章 - 知乎
